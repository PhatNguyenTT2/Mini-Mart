  ...
  const fetchSalesReport = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        setSalesData(response.data.data.products || []);
      ...
      setError(err.response?.data?.error || err.message || 'Failed to load sales report');
    } finally {
      setLoading(false);
    }
  };
        ...
        <div className="flex items-center gap-3">
          <div>
            ...
            <p className="text-[13px] text-gray-600 mt-1">
              View product sales statistics by date range
            </p>
          </div>
        </div>
      </div>
      ...
      {summary && (
        <div className="grid grid-cols-5 gap-4">
          ...
          <div className="bg-white rounded-lg shadow-sm p-4 border-l-4 border-orange-500">
            <p className="text-[11px] text-gray-600 font-medium uppercase tracking-wider">Products Sold</p>
