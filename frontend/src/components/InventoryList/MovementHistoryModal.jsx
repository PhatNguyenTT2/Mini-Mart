...
export const MovementHistoryModal = ({ isOpen, onClose, inventory }) => {
  ...
  const fetchMovements = async () => {
    ...
    try {
      ...
      const response = await inventoryService.getMovementHistory(productId, 100);
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-[13px] text-gray-600 font-['Poppins',sans-serif] mt-1">
              Product: #{inventory?.productId || inventory?.product?.id} - {inventory?.product?.name}
            </p>
          </div>
