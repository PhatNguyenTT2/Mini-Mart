...
class ChatSocketService {
  ...
  connect(token) {
    ...
    // Socket.IO path = '/ws/chat' (matches server config)
    // Vite proxy: /ws/chat → localhost:8080 → nginx → chatbot:3008  
    // Client sends handshake to GET /ws/chat/?EIO=4&transport=polling
    this.socket = io({
      path: '/ws/chat',
      auth: { token },
