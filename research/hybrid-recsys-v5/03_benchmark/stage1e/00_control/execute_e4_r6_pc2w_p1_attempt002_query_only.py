#!/usr/bin/env python3
"""Run authorized PC2W-P1 attempt-002 as one query-only start/query/stop probe.
...
RUNNER_RELATIVE = CONTROL_RELATIVE / "execute_e4_r6_pc2w_p1_attempt002_query_only.py"
AUTH_RELATIVE = CONTROL_RELATIVE / "e4_r6_pc2w_p1_attempt002_user_authorization_and_model_override.json"
...
DISPATCH_RELATIVE = CONTROL_RELATIVE / "rebaseline_v2_e4_r6_pc2w_p1_attempt002_dispatch.json"
...
def material_passport(created_at: str, auth: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "version_label": "stage1e_e4_r6_pc2w_p1_attempt002_execution_v1",
        "upstream_dependencies": [
            "stage1e_e4_r6_pc2w_p1_attempt002_user_authorization_v1",
...
def main() -> int:
    ...
    if dispatch.get("schema_version") != "stage1e-e4-r6-pc2w-p1-attempt002-dispatch-1.0":
        raise RuntimeError("attempt-002 dispatch v1 missing")
    ...
    if decision.get("decision") != "AUTHORIZE_PC2W_P1_ATTEMPT002_EXECUTION":
        raise RuntimeError("P1 attempt-002 user authorization missing")
    if decision.get("docker_stopped_attestation") != (
        "USER_CONFIRMED_ALL_DOCKER_STOPPED_BEFORE_ATTEMPT002"
    ):
        raise RuntimeError("attempt-002 Docker-stopped user attestation missing")
    false_auth = (
