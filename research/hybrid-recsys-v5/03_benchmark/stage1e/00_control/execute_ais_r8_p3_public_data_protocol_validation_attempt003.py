...
EXPECTED_OUTPUT_ROOT = "E:/UIT/cv/materialized-experiments/hybrid-recsys-v5/ai-service-v2/r8/public-data-protocol-validation/attempt-003"
...
def main() -> int:
    ...
    def validate_packet_attempt003(payload: dict[str, Any]) -> None:
        ...
        if runtime["output_root"] != EXPECTED_OUTPUT_ROOT:
            raise ValueError("unexpected attempt-003 output root")
        if runtime["container_name"] != "ais-r8-p3-public-validation-attempt-003":
            raise ValueError("unexpected attempt-003 container name")
    ...
    def static_preflight_attempt003(
        ...
        failure_audit = base.load_json(paths["attempt002_failure_audit"])
        if failure_audit.get("verdict") != "ATTEMPT002_PRETRAINING_ASSERTION_FAILURE_CONFIRMED":
            raise ValueError("Attempt-002 failure remediation authority is invalid")
        if failure_audit["runtime_evidence"].get("training_started") is not False:
            raise ValueError("Attempt-002 training-state authority drift")
    ...
    def build_docker_argv_attempt003(
        payload: dict[str, Any],
    ...
    base.validate_packet = validate_packet_attempt003
    base.static_preflight = static_preflight_attempt003
    base.build_docker_argv = build_docker_argv_attempt003
