...
DISPATCH_PATH = CONTROL / "rebaseline_v2_e4_r6_m0_m1_dispatch.json"
C1_VALIDATOR = CONTROL / "validate_e4_r6_c1_packet.py"
...
def common_preflight(require_both_absent: bool) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    if (
        dispatch.get("status") != "FROZEN_READY_FOR_CENTRAL_R6_M0_R6_M1_EXECUTION"
        or dispatch.get("entry_gate")
    ...
    receipt = strict_load(CONTROL / "rebaseline_v2_e4_r6_c1_validation_receipt.json")
    if (
    ...
    return dispatch, roots


def run_preflight() -> int:
    dispatch, roots = common_preflight(require_both_absent=True)
    validation = subprocess.run(
        [sys.executable, str(C1_VALIDATOR)],
        cwd=ROOT,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        check=False,
    ...
    if (
        ...
        or "PASS_R6_C1_EXACT_COMMAND_PACKET_READY_FOR_BOUNDED_R6_M0_R6_M1"
        not in stdout
    ):
        raise RuntimeError(
    ...
    result = {
        "schema_version": "stage1e-r6-m0-m1-central-preflight-result-1.0",
...
def run_lane(lane: str) -> int:
    ...
    summary = {
        "schema_version": "stage1e-r6-central-materialization-run-result-1.0",
