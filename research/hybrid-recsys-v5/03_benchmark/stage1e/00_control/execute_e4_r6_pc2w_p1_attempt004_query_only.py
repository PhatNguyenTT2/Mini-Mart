#!/usr/bin/env python3
"""Run PC2W-P1 attempt-004 as one fail-closed start/query-only/stop probe.
...
RUNNER_RELATIVE = CONTROL_RELATIVE / "execute_e4_r6_pc2w_p1_attempt004_query_only.py"
NATIVE_HELPER_RELATIVE = CONTROL_RELATIVE / "execute_e4_r6_pc2w_p1_attempt003_offline_equivalent_observation.py"
AUTH_RELATIVE = CONTROL_RELATIVE / "e4_r6_pc2w_p1_attempt004_user_authorization_and_model_override.json"
REQUIREMENTS_RELATIVE = CONTROL_RELATIVE / "e4_r6_pc2w_p1_docker_query_preflight_requirements.json"
CONTRACT_RELATIVE = CONTROL_RELATIVE / "e4_r6_pc2w_p1_attempt004_query_only_contract.json"
...
FAILURE_RECEIPT_RELATIVE = CONTROL_RELATIVE / "rebaseline_v2_e4_r6_pc2w_p1_attempt003_failure_receipt.json"
DISPATCH_RELATIVE = CONTROL_RELATIVE / "rebaseline_v2_e4_r6_pc2w_p1_attempt004_query_only_dispatch.json"
...
POST_STOP_SETTLING_SECONDS = 20
POST_STOP_SNAPSHOT_BARRIER_SECONDS = 5
...
def material_passport(created_at: str, auth: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "version_label": "stage1e_e4_r6_pc2w_p1_attempt004_query_only_execution_v1",
        "upstream_dependencies": [
            "stage1e_e4_r6_pc2w_p1_attempt004_user_authorization_standard_v1",
...
def main() -> int:
    ...
    failure_receipt = load_json(repo_root / FAILURE_RECEIPT_RELATIVE)
    if dispatch.get("schema_version") != "stage1e-e4-r6-pc2w-p1-attempt004-query-only-dispatch-1.0":
        raise RuntimeError("attempt-004 query-only dispatch schema mismatch")
    if auth.get("schema_version") != "stage1e-e4-r6-pc2w-p1-attempt004-user-authorization-standard-1.0":
        raise RuntimeError("attempt-004 Standard authorization schema mismatch")
    if contract.get("schema_version") != "stage1e-e4-r6-pc2w-p1-attempt004-query-only-contract-1.0":
        raise RuntimeError("attempt-004 query-only contract schema mismatch")
    ...
    if failure_receipt.get("fresh_independent_failure_packet_audit", {}).get("verdict") != "PASS_PC2W_P1_ATTEMPT003_FAILURE_PACKET_AUDITED_NO_RETRY":
        raise RuntimeError("attempt-003 audited failure packet missing")
    if failure_receipt.get("policy_facts", {}).get("new_retry_authorized") is not False:
        raise RuntimeError("attempt-003 failure receipt was altered")
    ...
    if decision.get("decision") != "AUTHORIZE_PC2W_P1_ATTEMPT004_START_QUERY_STOP_ON_VALIDATED_NATIVE_PASS":
        raise RuntimeError("P1 attempt-004 user authorization missing")
    ...
    if not isinstance(frozen, list) or len(frozen) != 7:
    ...
    time.sleep(POST_STOP_SETTLING_SECONDS)
    ...
    time.sleep(POST_STOP_SNAPSHOT_BARRIER_SECONDS)
    ...
    pass_conditions = {
        ...
        "prohibited_commands_absent": all(
            docker_subcommand_is_allowed(list(row.get("argv") or [])) for row in receipts
        ),
        "automatic_retry_count_zero": True,
    }
    verdict = (
        "PASS_PC2W_P1_ATTEMPT004_QUERY_EVIDENCE_READY_FOR_CENTRAL_G1"
        if all(value is True for value in pass_conditions.values())
        else "FAIL_CLOSED_PC2W_P1_ATTEMPT004_BACKEND_OR_POLICY_NOT_ADMISSIBLE"
    )
    ...
    command_document = {
        "schema_version": "stage1e-e4-r6-pc2w-p1-attempt004-command-receipts-1.0",
    ...
    inventory_document = {
        "schema_version": "stage1e-e4-r6-pc2w-p1-attempt004-runtime-inventory-1.0",
        "material_passport": passport,
    ...
    execution_document = {
        "schema_version": "stage1e-e4-r6-pc2w-p1-attempt004-execution-receipt-1.0",
        "material_passport": passport,
        "stage_id": "E4-R6-PC2W-P1-ATTEMPT004",
        ...
        "post_stop_stabilization": {
            "settling_seconds": POST_STOP_SETTLING_SECONDS,
            "snapshot_barrier_seconds": POST_STOP_SNAPSHOT_BARRIER_SECONDS,
    ...
    handoff_document = {
        "schema_version": "stage1e-e4-r6-pc2w-p1-attempt004-handoff-1.0",
        "material_passport": passport,
        "stage_id": "E4-R6-PC2W-P1-ATTEMPT004",
        "verdict": verdict,
