...
def assert_history_masking(spec: dict[str, Any]) -> dict[str, Any]:
    ...
    if type(valid.sampler).__name__ != "Sampler" or type(test.sampler).__name__ != "Sampler":
        raise ValueError("phase-aware Sampler was not selected")
    ...
    receipt = {
        "schema_version": "ais-r8-p3-attempt002-masking-semantics/1.0",
        "status": "PASS_BEFORE_TRAINING",
        "checked_at": now(),
        "attempt_id": ATTEMPT_ID,
        "resolved_repeatable": False,
        "validation_sampler": type(valid.sampler).__name__,
        "test_sampler": type(test.sampler).__name__,
...
def main() -> int:
    ...
    if wrapper_hash != os.environ["AIS_R8_EXPECTED_WRAPPER_SHA256"]:
        raise ValueError("attempt-002 wrapper hash mismatch")
    ...
    if sorted(path.name for path in output_root.iterdir()) != ["host_preflight.json"]:
        raise ValueError("attempt-002 output root is not fresh")
    ...
    manifest["attempt002_wrapper_sha256"] = wrapper_hash
            ...
            if not failure.exists():
                try:
                    write_json(
                        failure,
                        {
                            "schema_version": "ais-r8-p3-attempt002-failure/1.0",
