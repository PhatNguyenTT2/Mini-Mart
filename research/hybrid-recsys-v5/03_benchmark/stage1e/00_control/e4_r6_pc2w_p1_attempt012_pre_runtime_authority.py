#!/usr/bin/env python3
"""Pure Attempt-012 authority boundary used immediately before legacy main."""
...
MODEL_POLICY_SCHEMA = "stage1e-e4-r6-pc2w-p1-attempt012-model-policy-1.0"
AUTHORITY_SCHEMA = "stage1e-e4-r6-pc2w-p1-attempt012-pre-runtime-authority-1.0"
...
def validate_model_policy(
    ...
    """Apply one role-aware XHigh Standard policy shape."""
...
def _canonical_absolute(value: str, unbound_code: str) -> Path:
...
def build_expected_process_argv(contract: AuthorityContract) -> tuple[str, ...]:
    """Build expected argv from named contract parameters, never actual argv."""
...
def prepare_legacy_handoff(
    ...
    """Validate the public pre-legacy boundary and return its immutable binding."""
