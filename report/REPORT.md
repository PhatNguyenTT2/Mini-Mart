...
$$\text{sim}(q, d) = 1 - d_{\cos}(\vec{q}, \vec{d}) = \frac{\vec{q} \cdot \vec{d}}{|\vec{q}| \cdot |\vec{d}|}$$

Trong đó $\vec{q}$ là vector embedding của câu hỏi, $\vec{d}$ là vector embedding của sản phẩm (cùng 768 chiều).

**Keyword Search** — PostgreSQL Full-Text Search:

$$\text{score}_{\text{keyword}}(q, d) = \text{ts\_rank}(d.\text{fts\_content},\ \text{plainto\_tsquery}(\text{'simple'}, q))$$
...
| **CombMNZ** | $|\\{i: s_i(d)>0\\}| \cdot \sum s_i(d)$ | Ưu tiên items xuất hiện ở nhiều nguồn | Phức tạp hơn, vẫn cần normalize |
...
| Sản phẩm A | Sản phẩm B | $|A \cap B|$ | $|A|$ | $|B|$ |
|---|---|---|---|---|
...
$$\text{Cặp (Bia, Khô gà):}\quad \text{support} = \frac{12}{100} = 0.12,\quad \text{conf}(Bia \Rightarrow Khô) = \frac{12}{30} = 0.40,\quad \text{lift} = \frac{12 \times 100}{30 \times 18} = 2.22$$

$$\text{Cặp (Bò, Nấm):}\quad \text{support} = \frac{15}{100} = 0.15,\quad \text{conf}(Bò \Rightarrow Nấm) = \frac{15}{25} = 0.60,\quad \text{lift} = \frac{15 \times 100}{25 \times 20} = 3.00$$
...
- Dữ liệu mua hàng: $f(\text{purchase\_count}, \text{total\_quantity}, \text{recency})$
- Implicit feedback: $\text{clicks} \times 0.2 + \text{carts} \times 0.5 + \text{hovers} \times 0.05$
...
|---|---|---|
| **Công thức** | $\frac{\sum R_{u,i} R_{u,j}}{||i|| \cdot ||j||}$ | $\frac{\sum (R_{u,i}-\bar{R}_u)(R_{u,j}-\bar{R}_u)}{...}$ |
...
### 4.6 Testcase minh họa — Tệp khách hàng siêu thị

**Ma trận tương tác** ($R_{u,i}$ = interaction_score):

| | Bò (1) | Nấm (3) | Sữa (10) | Bia (17) |
|---|---|---|---|---|
| User A (SV cuối tháng) | 3.5 | 2.8 | 0 | 0 |
| User B (SV cuối tháng) | 4.0 | 3.2 | 0 | 1.5 |
| User C (Gia đình) | 0 | 0 | 2.0 | 3.0 |

**Tính sim(Bò, Nấm):**

$$\text{Common users} = \{A, B\} \quad (\geq 2 \text{ → đủ điều kiện})$$

$$\text{dotProduct} = 3.5 \times 2.8 + 4.0 \times 3.2 = 9.8 + 12.8 = 22.6$$

$$||\vec{R}_{\cdot,Bò}|| = \sqrt{3.5^2 + 4.0^2} = 5.315, \quad ||\vec{R}_{\cdot,Nấm}|| = \sqrt{2.8^2 + 3.2^2} = 4.252$$

$$\text{sim}(Bò, Nấm) = \frac{22.6}{5.315 \times 4.252} = 0.9998$$

→ Bò và Nấm có độ tương đồng cực cao — hai sản phẩm cùng cluster "nấu lẩu".

### 4.7 Độ phức tạp

| Giai đoạn | Độ phức tạp | Giải thích |
|---|---|---|
| Xây dựng ma trận $R_{u,i}$ | $O(n)$ | $n$ = số interactions |
| Tính norm $||\vec{R}_{\cdot,i}||$ | $O(m \times \bar{u})$ | $m$ = số items, $\bar{u}$ = avg users/item |
| Cosine Similarity all pairs | $O(m^2 \times \bar{c})$ | $\bar{c}$ = avg common users |
| Pruning (`common_users < 2`) | Giảm ~70-80% cặp | |
...
$$\text{final}(p) = \alpha \cdot \hat{c}(p) + \beta \cdot \hat{f}(p) + \gamma \cdot \text{conf}(A \Rightarrow p) + \delta \cdot \text{persona}(u)$$

Trong đó:
- $\hat{c}(p) = \frac{\text{rrf\_score}(p)}{\max(\text{rrf\_scores})}$ — Local Max normalization cho Content
- $\hat{f}(p) = \frac{\text{prediction\_score}(p)}{\max(\text{prediction\_scores})}$ — Local Max normalization cho CF
- $\text{conf}(A \Rightarrow p)$ — Confidence từ Apriori, đã trong khoảng $[0,1]$
...
**Feedback data (30 ngày):**

| Source | Recommended | Hovered | Clicked | Cart | Purchased |
|---|---|---|---|---|---|
| content | 800 | 200 | 120 | 30 | 8 |
| cf | 250 | 80 | 40 | 15 | 5 |
| apriori | 150 | 35 | 15 | 8 | 3 |

**Tính Weighted Conversion Scores:**

$$\text{content}: 8 \times 1.0 + 30 \times 0.5 + 120 \times 0.2 + 200 \times 0.1 = 67 \quad \Rightarrow \text{rate} = \frac{67}{800} = 0.0838$$

$$\text{cf}: 5 \times 1.0 + 15 \times 0.5 + 40 \times 0.2 + 80 \times 0.1 = 28.5 \quad \Rightarrow \text{rate} = \frac{28.5}{250} = 0.1140$$

$$\text{apriori}: 3 \times 1.0 + 8 \times 0.5 + 15 \times 0.2 + 35 \times 0.1 = 13.5 \quad \Rightarrow \text{rate} = \frac{13.5}{150} = 0.0900$$

**Raw weights** ($\delta=0.10$, available $= 0.90$, $\sum \text{rates} = 0.2878$):

$$\alpha_{\text{raw}} = \frac{0.0838}{0.2878} \times 0.90 = 0.2620, \quad \beta_{\text{raw}} = 0.3565, \quad \gamma_{\text{raw}} = 0.2815$$

**Exponential Smoothing** ($\lambda=0.8$):

$$\alpha_{\text{new}} = 0.8 \times 0.40 + 0.2 \times 0.2620 = 0.3724$$
$$\beta_{\text{new}} = 0.8 \times 0.25 + 0.2 \times 0.3565 = 0.2713$$
$$\gamma_{\text{new}} = 0.8 \times 0.25 + 0.2 \times 0.2815 = 0.2563$$
...
|---|---|---|---|
| **Precision@K** | $\frac{\text{số SP đúng trong top K}}{K}$ | Tỷ lệ kết quả phù hợp trong top K | RAG + RRF |
| **Recall@K** | $\frac{\text{số SP đúng trong top K}}{\text{tổng SP đúng}}$ | Độ bao phủ | RAG + RRF |
| **NDCG** | $\frac{DCG@K}{IDCG@K}$ với $DCG = \sum_{i=1}^{K} \frac{2^{rel_i} - 1}{\log_2(i+1)}$ | Chất lượng xếp hạng — vị trí cao hơn = giá trị hơn | Hybrid Ensemble |
| **MRR** | $\frac{1}{|Q|} \sum_{q=1}^{|Q|} \frac{1}{\text{rank}_q}$ | Vị trí trung bình của kết quả đúng đầu tiên | RAG |
| **Hit Rate@K** | $\frac{\text{số users mua ≥ 1 SP gợi ý}}{|\text{users}|}$ | Tỷ lệ "chạm" | CF |
| **Conversion Rate** | $\frac{n_{\text{purchased}}}{n_{\text{recommended}}}$ | Gợi ý → Mua hàng | Ensemble tổng hợp |
