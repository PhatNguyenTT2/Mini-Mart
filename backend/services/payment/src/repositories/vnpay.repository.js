...
class VNPayRepository {
     ...
     async create(data) {
         const { payment_id, reference_id, vnp_txn_ref, vnp_amount, payment_url, origin } = data;
         const query = `
             INSERT INTO vnpay_transaction 
             (payment_id, reference_id, vnp_txn_ref, vnp_amount, payment_url, origin, status)
             VALUES ($1, $2, $3, $4, $5, $6, 'pending')
             RETURNING *
         `;
         const { rows } = await this.pool.query(query, [
             payment_id, reference_id, vnp_txn_ref, vnp_amount, payment_url, origin || 'pos'
         ]);
         return rows[0];
     }
