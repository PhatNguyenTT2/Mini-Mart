...
function createPaymentRouter(paymentService) {
    ...
    router.get('/vnpay/return', async (req, res) => {
        try {
            ...
            if (origin === 'customer') {
                const customerUrl = (process.env.CUSTOMER_URL || 'http://localhost:5174').replace(/\/$/, '');
                if (isSuccess) {
                    return res.redirect(`${customerUrl}/order-status/${orderId}?payment=success`);
                } else {
                    return res.redirect(`${customerUrl}/order-status/${orderId}?payment=failed&code=${vnpResponseCode}`);
                }
            }
