...
class OrderService {
   ...
   */
  async fetchBatchesFromInventory(storeId, productId, jwtToken) {
  ...
  async allocateBatchesFEFO(storeId, items, jwtToken) {
    ...
    for (const item of items) {
      ...
      const batches = await this.fetchBatchesFromInventory(storeId, item.product_id, jwtToken);
    ...
    try {
      ...
      if (status && order.delivery_type === 'delivery') {
        ...
        if (status === 'shipping' && order.status === 'draft') {
          ...
          const shippingItems = details.map(d => ({
            productId: d.product_id,
            productName: d.product_name,
            batchId: d.batch_id,
            quantity: d.quantity,
          ...
          await outbox.insertEvent(client, EVENT.ORDER_SHIPPING, {
            orderId: id,
            storeId,
            customerId: order.customer_id,
            items: shippingItems,
