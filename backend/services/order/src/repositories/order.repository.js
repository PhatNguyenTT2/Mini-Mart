...
class OrderRepository {
  ...
  async findAll(storeId, filters = {}) {
    let query = 'SELECT sale_order.*, (SELECT COALESCE(SUM(quantity), 0) FROM sale_order_detail WHERE order_id = sale_order.id) as item_count FROM sale_order WHERE store_id = $1';
  ...
  async createOrderWithClient(client, storeId, data, status = 'draft') {
    const { customer_id, created_by, delivery_type, address, shipping_fee, discount_percentage, total_amount, coupon_code, coupon_discount } = data;
    const query = `
        INSERT INTO sale_order 
        (store_id, customer_id, created_by, delivery_type, address, shipping_fee, discount_percentage, total_amount, status, payment_status, coupon_code, coupon_discount)
        VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9, 'pending', $10, $11)
        RETURNING *
      `;
    const { rows } = await client.query(query, [
      storeId, customer_id, created_by, delivery_type, address, shipping_fee || 0, discount_percentage || 0, total_amount || 0, status, coupon_code || null, coupon_discount || 0
    ]);
    return rows[0];
  }
