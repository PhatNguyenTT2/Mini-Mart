...
class CoPurchaseRepository {
    ...
    async getRelatedProducts(productId, storeId, minCount = 3) {
        const { rows } = await this.pool.query(`
            SELECT co_purchase_stats.product_id_b, co_purchase_stats.co_purchase_count,
                   COALESCE(confidence_ab, 0) AS confidence,
                   COALESCE(lift, 0) AS lift,
                   COALESCE(substring(pb.content from 'Sản phẩm "([^"]+)"'), 'Product ' || co_purchase_stats.product_id_b) AS product_name_b
            ...
            WHERE co_purchase_stats.product_id_a = $1 AND co_purchase_stats.store_id = $2
              AND co_purchase_stats.co_purchase_count >= $3
              AND (lift > 1 OR lift = 0)
            UNION ALL
            SELECT co_purchase_stats.product_id_a, co_purchase_stats.co_purchase_count,
                   COALESCE(confidence_ba, 0) AS confidence,
                   COALESCE(lift, 0) AS lift,
                   COALESCE(substring(pa.content from 'Sản phẩm "([^"]+)"'), 'Product ' || co_purchase_stats.product_id_a) AS product_name_b
            FROM co_purchase_stats
            ...
            WHERE co_purchase_stats.product_id_b = $1 AND co_purchase_stats.store_id = $2
              AND co_purchase_stats.co_purchase_count >= $3
              AND (lift > 1 OR lift = 0)
