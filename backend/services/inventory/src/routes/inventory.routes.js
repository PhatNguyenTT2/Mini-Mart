...
function createInventoryRouter(inventoryService, inventoryRepo, { catalogServiceUrl }) {
    ...
    /**
     * GET /public/summary/:storeId — Public inventory summary for Customer Web
     * PUBLIC: No auth required. Returns only productId and totalOnShelf.
     */
    router.get('/public/summary/:storeId', async (req, res, next) => {
        try {
            const storeId = parseInt(req.params.storeId);
            if (!storeId) {
