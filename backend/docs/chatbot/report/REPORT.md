...
| `interaction_score` | NUMERIC | **R[u,i]** — điểm tương tác tổng hợp. Được tính từ hai nguồn: (1) dữ liệu mua hàng: `f(purchase_count, total_quantity, recency)`, và (2) implicit feedback từ Dual-Tracking: `clicks×0.2 + carts×0.5 + hovers×0.05`. Đây là giá trị trực tiếp dùng trong Cosine Similarity (xem Mục 8.4) |
...
| `store_id` | BIGINT | Chi nhánh |
| `source` | TEXT | **Nguồn thuật toán** đã gợi ý sản phẩm này: `'content'`, `'cf'`, `'apriori'`, hoặc `'organic'` (tương tác tự nhiên ngoài chatbot — xem Mục 8.4). Weight Learner dùng field này để tính conversion rate per source, loại trừ `organic` |
| `action` | TEXT | **Hành động**: `'recommended'` → `'hovered'` → `'clicked'` → `'added_to_cart'` → `'purchased'`. Phễu chuyển đổi 5 bước |
...
Hệ thống tracking ban đầu chỉ thu thập dữ liệu từ luồng Chatbot (sản phẩm do AI gợi ý). Điều này tạo ra **bài toán Data Sparsity** cho thuật toán Item-based Collaborative Filtering: ma trận tương tác `R[u,i]` chỉ có dữ liệu mua hàng, thiếu các tín hiệu hành vi phụ trợ (click, hover, add-to-cart) từ hoạt động duyệt web tự nhiên (organic).

#### 8.4.2 Kiến trúc Dual-Tracking

Giải pháp: Duy trì **hai luồng dữ liệu độc lập** trên cùng một bảng `recommendation_feedback`, phân biệt bằng trường `source`:
...
flowchart LR
    ...
    subgraph "Luồng 2: Global Tracking (Organic)"
        C2["ProductCard.jsx<br/>(Home, Danh mục, Tìm kiếm)"] -->|"source = organic"| FB
    end

    FB -->|"WHERE source IN (content,cf,apriori)"| WL["Weight Learner<br/>(Chỉ đánh giá AI)"]
    FB -->|"WHERE source = ANY<br/>(bao gồm organic)"| CF["CF Interaction Matrix<br/>(Giải quyết Data Sparsity)"]
...
|-----------|--------------------------|---------------------------|
| Phạm vi thu thập | `ChatProductCard.jsx` | `ProductCard.jsx` (toàn bộ Customer Page) |
| Giá trị `source` | `content`, `cf`, `apriori` | `organic` |
| Mục đích | Đánh giá hiệu quả từng thuật toán AI | Thu thập Implicit Feedback, lấp đầy ma trận thưa |
| Thuật toán phục vụ | Adaptive Weight Learning (α,β,γ) | Item-based Collaborative Filtering |

#### 8.4.3 Cách ly dữ liệu (Data Isolation)

**Weight Learner** chỉ sử dụng dữ liệu chatbot để tính conversion rate:

```sql
SELECT source, COUNT(*) FILTER (WHERE action = 'recommended') AS recommended, ...
FROM recommendation_feedback
WHERE store_id = $1 AND created_at > NOW() - INTERVAL '1 day' * $2
  AND source IN ('content', 'cf', 'apriori')  -- Loại trừ organic
GROUP BY source
```
...
```
R[u,i] = (Lượt_Mua × 1.0) + (Vào_Giỏ × 0.5) + (Click × 0.2) + (Hover ≥1.5s × 0.05)
```

#### 8.4.4 Giải quyết Bug định danh (Guest Fallback)

Hệ thống feedback API ban đầu phụ thuộc vào `req.body.userId` do Frontend truyền. Tuy nhiên `chatFeedbackService.js` không gửi trường này, dẫn đến `user_id = NULL` trong database và hiển thị "Guest" trên Dashboard.

**Giải pháp**: Backend tự trích xuất `userId` từ JWT token trong `Authorization` header (đã được API interceptor tự gắn). Phương pháp này:
- **An toàn hơn**: Tránh Frontend tự truyền ID có thể bị giả mạo
- **Không cần sửa Frontend**: Tương thích ngược, mọi feedback call hiện có tự động được gắn đúng userId
