...
### 5.5 Phương án Cải tiến: Hệ Thống Theo Dõi Kép (Dual-Tracking System)

"Kính thưa Hội đồng, ngoài tín hiệu Hover Dwell, em xin trình bày thêm một cải tiến kiến trúc quan trọng: **Hệ thống Theo dõi Kép (Dual-Tracking System)**."

#### Bài toán Data Sparsity

"Hệ thống tracking ban đầu chỉ thu thập dữ liệu từ luồng Chatbot — tức là chỉ khi AI gợi ý sản phẩm. Điều này tạo ra **bài toán Ma trận thưa (Data Sparsity)** cho thuật toán Collaborative Filtering: ma trận tương tác R[u,i] chỉ có dữ liệu từ các đơn hàng đã hoàn thành, trong khi hàng nghìn hành vi duyệt web (click, hover) bị bỏ phí."

#### Giải pháp: Hai Luồng Dữ Liệu Độc Lập

| | Luồng 1: Local (Chatbot) | Luồng 2: Global (Organic) |
|---|---|---|
| **Nguồn** | Thẻ sản phẩm trong Chatbot | Toàn bộ Customer Page |
| **Source tag** | `content`, `cf`, `apriori` | `organic` |
| **Phục vụ** | Weight Learner (α,β,γ) | CF Interaction Matrix |

"Điểm quan trọng nhất trong thiết kế này là **cách ly dữ liệu (Data Isolation)**:
- Weight Learner — thuật toán học trọng số — **chỉ** sử dụng dữ liệu chatbot. Nếu trộn organic vào, conversion rate sẽ bị pha loãng vì organic traffic không có bước 'recommended', làm sai lệch đánh giá hiệu quả AI.
- Trong khi đó, thuật toán CF **tận dụng cả hai luồng** để làm dày ma trận tương tác. Công thức cập nhật: `R[u,i] = Mua×1.0 + Giỏ×0.5 + Click×0.2 + Hover×0.05`."

#### Giải quyết Bug Định danh

"Trong quá trình triển khai, em phát hiện một lỗi kỹ thuật: Frontend gửi feedback nhưng **không truyền userId** — dẫn đến mọi record đều hiển thị 'Guest' trên Dashboard. Em đã sửa bằng cách cho Backend **tự trích xuất userId từ JWT token** trong header Authorization, thay vì phụ thuộc vào Frontend. Phương pháp này vừa an toàn hơn (tránh giả mạo), vừa tương thích ngược."

### 5.6 Phương hướng mở rộng tương lai
